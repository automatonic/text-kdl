...
namespace Automatonic.Text.Kdl.Serialization.Metadata
{
    ...
    internal sealed class KdlPropertyInfo<T> : KdlPropertyInfo
    {
        ...
        internal KdlPropertyInfo(Type declaringType, KdlTypeInfo? declaringTypeInfo, KdlSerializerOptions options)
        ...
        internal override bool GetMemberAndWriteKdl(object obj, ref WriteStack state, KdlWriter writer)
        {
            T value = Get!(obj);
            ...
            if (value is null)
            {
                ...
                {
                    ThrowHelper.ThrowKdlException_PropertyGetterDisallowNull(Name, state.Current.KdlTypeInfo.Type);
                }
                ...
                if (EffectiveConverter.HandleNullOnWrite)
                {
                    ...
                    {
                        ThrowHelper.ThrowKdlException_SerializationConverterWrite(EffectiveConverter);
                    }
                }
                else
                {
        ...
        internal override bool GetMemberAndWriteKdlExtensionData(object obj, ref WriteStack state, KdlWriter writer)
        {
            bool success;
            T value = Get!(obj);
        ...
        internal override bool ReadKdlAndSetMember(object obj, scoped ref ReadStack state, ref KdlReader reader)
        {
            bool success;
            ...
            if (isNullToken && !EffectiveConverter.HandleNullOnRead && !state.IsContinuation)
            {
                if (default(T) is not null || !CanDeserialize)
                {
                    ...
                    {
                        Debug.Assert(CanDeserialize || EffectiveObjectCreationHandling == KdlObjectCreationHandling.Populate);
                        ThrowHelper.ThrowInvalidOperationException_DeserializeUnableToAssignNull(EffectiveConverter.Type);
                    }

                    ThrowHelper.ThrowKdlException_DeserializeUnableToConvertValue(EffectiveConverter.Type);
                }
                ...
                if (!IgnoreNullTokensOnRead)
                {
                    ...
                    {
                        ThrowHelper.ThrowKdlException_PropertySetterDisallowNull(Name, state.Current.KdlTypeInfo.Type);
                    }
            ...
            else if (EffectiveConverter.CanUseDirectReadOrWrite && state.Current.NumberHandling == null)
            {
                ...
                Debug.Assert(EffectiveObjectCreationHandling != KdlObjectCreationHandling.Populate, "Populating should not be possible for simple types");
        ...
        internal override bool ReadKdlAsObject(scoped ref ReadStack state, ref KdlReader reader, out object? value)
        {
            bool success;
            bool isNullToken = reader.TokenType == KdlTokenType.Null;
            if (isNullToken && !EffectiveConverter.HandleNullOnRead && !state.IsContinuation)
        ...
        private protected override void ConfigureIgnoreCondition(KdlIgnoreCondition? ignoreCondition)
        {
