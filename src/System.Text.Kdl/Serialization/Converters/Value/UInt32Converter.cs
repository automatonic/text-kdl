...
namespace System.Text.Kdl.Serialization.Converters
{
    internal sealed class UInt32Converter : KdlPrimitiveConverter<uint>
    {
        ...
        public override uint Read(ref KdlReader reader, Type typeToConvert, KdlSerializerOptions options)
        {
        ...
        internal override uint ReadAsPropertyNameCore(ref KdlReader reader, Type typeToConvert, KdlSerializerOptions options)
        {
            Debug.Assert(reader.TokenType == KdlTokenType.PropertyName);
            return reader.GetUInt32WithQuotes();
        }

        internal override void WriteAsPropertyNameCore(KdlWriter writer, uint value, KdlSerializerOptions options, bool isWritingExtensionDataProperty)
        {
            writer.WritePropertyName(value);
        }

        internal override uint ReadNumberWithCustomHandling(ref KdlReader reader, KdlNumberHandling handling, KdlSerializerOptions options)
        {
        ...
        internal override void WriteNumberWithCustomHandling(KdlWriter writer, uint value, KdlNumberHandling handling)
        {
